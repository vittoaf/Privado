...
DROP TABLE IF EXISTS `rs-nprd-dlk-dt-stg-mica-4de1.delivery_canales.cotizacion_journey_tmp_vitto_anterior`;
DROP TABLE IF EXISTS `rs-nprd-dlk-dt-stg-mica-4de1.delivery_canales.cotizacion_journey_tmp_vitto`;
...
CREATE TABLE  `rs-nprd-dlk-dt-stg-mica-4de1.delivery_canales.cotizacion_journey_tmp_vitto_anterior`
AS
...
SELECT
	...
	pre.cod_prod,
	CASE
		WHEN cot.estado = 3 THEN CAST(cot.id_cotizacion AS STRING) --CAST(IFNULL(cot.numero_poliza,cot.id_cotizacion) AS STRING)
