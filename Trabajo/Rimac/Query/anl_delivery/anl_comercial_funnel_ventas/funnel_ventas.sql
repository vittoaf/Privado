...
TRUNCATE TABLE `{project_id_anl_msrl}.anl_comercial.funnel_ventas`;

INSERT INTO `{project_id_anl_msrl}.anl_comercial.funnel_ventas`
WITH 
t_producto AS (
	SELECT 
		p.id_producto, 
	...
	FROM (
		SELECT 
			...
			p.nom_corto
		FROM 
			`{project_id_stg}.stg_modelo_producto.producto` AS p
...
t_tramite_nuevas_polizas_renov as (
	...
	FROM 
	      `{project_id_stg_msrl}.stg_modelo_operaciones.tramite` t
...
t_tramites AS (
	...
	LEFT JOIN `{project_id_stg}.stg_modelo_comercial.intermediario_mc` it 
		ON (it.id_intermediario = t.id_intermediario)
	LEFT JOIN t_producto prod 
		ON (prod.id_producto = t.id_producto) 
	LEFT JOIN `{project_id_stg}.stg_modelo_poliza.poliza` pol 
		ON (pol.id_poliza = t.id_poliza) 
	LEFT JOIN `{project_id_stg}.stg_modelo_persona.persona` cont 
...
t_cot_mnt_data AS (
	...
	FROM 
	     `{project_id_stg_msrl}.stg_modelo_prospeccion.cotizacion` a
...
t_cotizaciones_prima_prod_xasesor AS (
	...
	FROM 
		`{project_id_anl_msrl}.anl_comercial.productividad` a
