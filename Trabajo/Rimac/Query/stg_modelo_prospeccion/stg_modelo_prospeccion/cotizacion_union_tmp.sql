--DROP TABLE IF EXISTS `rs-nprd-dlk-dt-stg-msrl-d0d7.stg_modelo_prospeccion.cotizacion_union_tmp`;
--DROP TABLE IF EXISTS `rs-nprd-dlk-dd-stgz-8ece.stg_modelo_prospeccion.cotizacion_union_tmp`;
DROP TABLE IF EXISTS `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_union_tmp`;

--CREATE TABLE `rs-nprd-dlk-dt-stg-msrl-d0d7.stg_modelo_prospeccion.cotizacion_union_tmp`
--CREATE TABLE `rs-nprd-dlk-dd-stgz-8ece.stg_modelo_prospeccion.cotizacion_union_tmp`
CREATE TABLE `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_union_tmp`
AS
SELECT
	...
	ARRAY_AGG(j.cot_nro_poliza ORDER BY j.fechora_actualizacion_estado_asesoria DESC, j.id_cotizacion_origen DESC)[OFFSET(0)] AS numpol,
	--Variables nuevas para COTSAS
...
FROM 
	...
	`{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_journey_tmp` j
GROUP BY
	j.codigo_asesor,
	j.email_asesor,
	j.nro_doc_asesor,
...
FROM 
	...
	`{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_cotweb_tmp` j
GROUP BY
	-- j.codigo_asesor,
	-- j.email_asesor,
	-- j.nro_doc_asesor,
...
SELECT
	...
	periodo
FROM 
	`{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_sas_tmp` j
...
SELECT
	...
	periodo
FROM 
`{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_finrisk_tmp` j
