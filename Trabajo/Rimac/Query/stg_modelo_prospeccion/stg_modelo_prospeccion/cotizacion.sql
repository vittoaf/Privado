--INSERT INTO `rs-nprd-dlk-dt-stg-msrl-d0d7.stg_modelo_prospeccion.cotizacion`
--INSERT INTO `rs-nprd-dlk-dd-stgz-8ece.stg_modelo_prospeccion.cotizacion`
INSERT INTO `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion`
...
personas_correo_data AS (
	...
	FROM 
		...
		`{project_id_stg}.stg_modelo_persona.colaborador` c
		...
		LEFT JOIN `{project_id_stg}.stg_modelo_persona.persona` a
			ON (a.id_persona=c.id_persona)
		CROSS JOIN UNNEST (a.documento_identidad) b
...
jerarquia_consolidado_vigente_data AS (
	...
	FROM 
		...
		`{project_id_raw}.de__canales.jerarquia_consolidado_vigente` b
	WHERE 
...
tramites_data AS (
	...
	FROM 
		...
		`{project_id_raw}.bdwf__appnote.TRAMITE` trm
...
tipo_cambio_data AS (
	...
	FROM 
		...
		`{project_id_raw}.prod__acselx.TASA_CAMBIO` tc
	WHERE 
		tipotasa = 'M' 
		AND codmoneda = 'USD' 
...
producto_ax_data AS (
	...
	FROM `{project_id_stg}.stg_modelo_producto.producto` AS pr
...
canal_data AS (
	...
	FROM `{project_id_stg}.stg_modelo_comercial.estructura_canal` ec
...
FROM 
	...
	`{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_union_tmp` a
