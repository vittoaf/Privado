...
CREATE TABLE `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_sas_tmp` AS
WITH COTIZACION AS (
SELECT *
FROM `{project_id_raw}.bdsas__app_iaa_acuerdo.ACU_ACUERDO`
...
from `{project_id_raw}.bdsas__app_iaa_tercero.TER_ROL` R
)
, CERTIFICADO as (
SELECT 
	ideprod,
...
FROM 
`{project_id_raw}.de__canales.bsa_base_ajustes_sas`
GROUP BY 
...
FROM `{project_id_raw}.bdsas__app_iaa_acuerdo.ACU_ROLACUERDO` A
INNER JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_ROLTERCERO` Y ON (A.iderolter = Y.iderolter)
INNER JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_TERCERO`    T ON (T.idetercero = Y.idetercero)
INNER JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_DOCTERCERO` TD ON (TD.idetercero = T.idetercero AND TD.idedoctercero = T.idedocterprincipal)
LEFT JOIN  `{project_id_raw}.bdsas__app_iaa_comunes.CFG_PARAMETRO` C ON ((C.idetippar = 'TER_TIPODOCUMENTO') AND C.codigoc = CAST(TD.idptipodocumento AS STRING))
...
SELECT 
	...
	*
FROM `{project_id_raw}.bdsas__app_iaa_acuerdo.ACU_ACUERDO` a
...
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_ROLTERCERO` b on a.idecanal=b.iderolter
INNER JOIN ROL c on b.iderol=c.iderol
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_TERCERO` d on b.idetercero=d.idetercero
LEFT JOIN  `{project_id_raw}.bdsas__app_iaa_tercero.PTO_MEDIOCONTACTO` PM ON (PM.IDEPUNTOCONTACTO = d.IDEMAILPRINCIPAL)
GROUP BY
	a.fecsts,
	a.ideacuerdo,
	a.ideprod,
...
FROM `{project_id_raw}.bdsas__app_iaa_acuerdo.ACU_INSPLAN`
...
SELECT 
	...
	a.numpol 													as numpol
FROM FINAL a
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_ROLTERCERO` b on a.idevendedor=b.iderolter
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_TERCERO` c on b.idetercero=c.idetercero
LEFT JOIN  `{project_id_raw}.bdsas__app_iaa_tercero.PTO_MEDIOCONTACTO` PM ON (PM.IDEPUNTOCONTACTO = c.IDEMAILPRINCIPAL)
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_producto.PRO_PRODUCTO` d on a.ideprod=d.ideprod
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_DOCTERCERO` TD ON (TD.idetercero = c.idetercero AND TD.idedoctercero = c.idedocterprincipal)
LEFT JOIN  `{project_id_raw}.bdsas__app_iaa_comunes.CFG_PARAMETRO` TDOC ON (TDOC.idetippar = 'TER_TIPODOCUMENTO' AND TRIM(TDOC.codigoc) = CAST(TD.idptipodocumento AS STRING))
LEFT JOIN TER_TITULAR       TIT ON (A.ideacuerdo = TIT.ideacuerdo) 
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_comunes.CFG_PARAMETRO` cfg_prm_acu	ON (cfg_prm_acu.idetippar = 'ACU_STSACUERDO_COTIZACION' AND cfg_prm_acu.codigon = A.stsacuerdo	)
LEFT JOIN (
			SELECT 
				IDEACUERDO,
				ARRAY_AGG(idpfrecuencia ORDER BY NUMOPER DESC)[OFFSET(0)] AS idpfrecuencia
  			FROM `{project_id_raw}.bdsas__app_iaa_finanzas.PRV_PLANIFICADORFINANCIERO`
			GROUP BY 
				IDEACUERDO
			) F ON A.ideacuerdo = F.IDEACUERDO
LEFT JOIN `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_origenCanal
	ON (
...
LEFT JOIN poliza pol on cer.idemaestro= pol.ideacuerdo
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_ROLTERCERO` b on cer.idecanal=b.iderolter
INNER JOIN ROL c on b.iderol=c.iderol
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_TERCERO` d on b.idetercero=d.idetercero
LEFT JOIN  `{project_id_raw}.bdsas__app_iaa_tercero.PTO_MEDIOCONTACTO` PM ON (PM.IDEPUNTOCONTACTO = d.IDEMAILPRINCIPAL)
GROUP BY 
...
,FINAL_CERT AS(
	...
	a.refexterna_cer
FROM previa a
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_ROLTERCERO` b on a.idevendedor_cer=b.iderolter
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_TERCERO` c on b.idetercero=c.idetercero
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.PTO_MEDIOCONTACTO` PM ON (PM.IDEPUNTOCONTACTO = c.IDEMAILPRINCIPAL)
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_producto.PRO_PRODUCTO` d on a.ideprod=d.ideprod
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_tercero.TER_DOCTERCERO` TD ON (TD.idetercero = c.idetercero AND TD.idedoctercero = c.idedocterprincipal)
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_comunes.CFG_PARAMETRO` TDOC ON (TDOC.idetippar = 'TER_TIPODOCUMENTO' AND TRIM(TDOC.codigoc) = CAST(TD.idptipodocumento AS STRING))
LEFT JOIN TER_TITULAR       TIT ON (A.ideacuerdo = TIT.ideacuerdo) 
LEFT JOIN `{project_id_raw}.bdsas__app_iaa_comunes.CFG_PARAMETRO` cfg_prm_acu	ON (cfg_prm_acu.idetippar = 'ACU_STSACUERDO_CERTIFICADO' AND cfg_prm_acu.codigon = A.stsacuerdo	)
LEFT JOIN (
			SELECT 
				IDEACUERDO,
				ARRAY_AGG(idpfrecuencia ORDER BY NUMOPER DESC)[OFFSET(0)] AS idpfrecuencia
