...
CREATE TABLE `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_finrisk_tmp` as 
...
FROM `{project_id_raw}.bdweb__app_cotirrvv.COT_CODIGOS` g
order by 2
),
ESTADO_RENTA_GARANTIZADA AS (
select ID_COD, DESC_COD, 
CASE WHEN DESC_COD = 'Emitida' THEN 1 ELSE 2 END REGLA,
FROM `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_CODIGOS` g
order by 2
),
...
from    `{project_id_raw}.bdweb__app_cotirrvv.COT_COTIZACIONES`   a
inner join  `{project_id_raw}.bdweb__app_cotirrvv.COT_SOL_MELER` b on a.ID_SOL_MELER = b.ID_SOL_MELER
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_PROD` d  on a.id_sol_meler = d.id_sol_meler and a.num_opcion  = d.num_opcion
left join `{project_id_raw}.bdweb__app_cotirrvv.COT_GANADAS` Gan  on Gan.ID_COT=a.ID_COT and Gan.ID_VALIDO='S'
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_CODIGOS` c on b.id_sexo      = c.id_cod
inner join ESTADO_RENTA_VITALICIA g on g.ID_COD=b.ID_ESTADO_SOLIC
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_CODIGOS` h on h.ID_COD=b.ID_DOC
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_USUARIOS` usu on b.ID_ASESOR_ASOCIADO=usu.ID_USUARIO
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_CODIGOS` Pe on Pe.ID_COD=usu.ID_PERFIL
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_CODIGOS` Mo on Mo.ID_COD=b.ID_MONEDA
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_CODIGOS` Prod on Prod.ID_COD=d.ID_MODALIDAD
...
select
  ...
  from  
`{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_COTIZACION` a 
inner join (select max(vers) Max_vers_cot,ID_COTIZACION from `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_COTIZACION` group by ID_COTIZACION) Ul_Cot on Ul_Cot.Max_vers_cot=a.Vers and Ul_Cot.ID_COTIZACION=a.ID_COTIZACION
inner join `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_COTIZACION_OPCION` b on b.id_cotizacion=a.Id_cotizacion
inner join (select max(VERSOPCION) Max_vers_cotop,ID_COTIZACION, Opcion from `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_COTIZACION_OPCION` group by ID_COTIZACION,OPCION) Ul_CotOp on Ul_CotOp.Max_vers_cotop=b.VERSOPCION and Ul_CotOp.ID_COTIZACION=b.ID_COTIZACION and Ul_CotOp.OPCION=b.OPCION
inner join `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_PLAN` pl on pl.id_plan=b.id_plan and pl.vers=b.vers_plan
inner join `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_CODIGOS` c on a.Aseg_id_sexo      = c.id_cod
inner join ESTADO_RENTA_GARANTIZADA  g on g.ID_COD=a.ID_ESTADO
inner join `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_CODIGOS`  h on h.ID_COD=a.ASEG_TIPO_DOC
inner join `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_CODIGOS`  i on i.ID_COD=b.ID_ESTADOOPCION
inner join `{project_id_raw}.bdweb__app_cotirrvv.COT_USUARIOS` usu on a.ID_AGENTEVENTAS=usu.ID_USUARIO 
left join `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_CODIGOS`  Pe on usu.ID_PERFIL_PRIVADA = Pe.ID_COD
inner join `{project_id_raw}.bdweb__app_cotirrvv.PRIVADA_CODIGOS`  Mo on Mo.ID_COD=b.ID_MONEDAPRIMA  
where  1=1
...
left join   `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_tipdoc
...
left join   `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_prod
...
left join   `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_estcot
...
left join   `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_moneda
