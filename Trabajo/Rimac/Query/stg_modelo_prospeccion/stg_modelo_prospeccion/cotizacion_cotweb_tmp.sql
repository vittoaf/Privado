--DROP TABLE IF EXISTS `rs-nprd-dlk-dt-stg-msrl-d0d7.stg_modelo_prospeccion.cotizacion_cotweb_tmp`;
--DROP TABLE IF EXISTS `rs-nprd-dlk-dd-stgz-8ece.stg_modelo_prospeccion.cotizacion_cotweb_tmp`;
DROP TABLE IF EXISTS `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_cotweb_tmp`;

--CREATE TABLE `rs-nprd-dlk-dt-stg-msrl-d0d7.stg_modelo_prospeccion.cotizacion_cotweb_tmp`
--CREATE TABLE `rs-nprd-dlk-dd-stgz-8ece.stg_modelo_prospeccion.cotizacion_cotweb_tmp`
CREATE TABLE `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_cotweb_tmp`
AS
WITH unnested_data AS (
	...
	FROM 
		...
		`{project_id_raw}.de__canales.TMP_COTIZADOR` a
...
FROM 
	...
	LEFT JOIN `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_codprod
		ON (
