--DROP TABLE IF EXISTS `rs-nprd-dlk-dt-stg-msrl-d0d7.stg_modelo_prospeccion.cotizacion_journey_tmp`;
--DROP TABLE IF EXISTS `rs-nprd-dlk-dd-stgz-8ece.stg_modelo_prospeccion.cotizacion_journey_tmp`;
DROP TABLE IF EXISTS `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_journey_tmp`;

--CREATE TABLE `rs-nprd-dlk-dt-stg-msrl-d0d7.stg_modelo_prospeccion.cotizacion_journey_tmp`
--CREATE TABLE `rs-nprd-dlk-dd-stgz-8ece.stg_modelo_prospeccion.cotizacion_journey_tmp`
CREATE TABLE `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion_journey_tmp`
AS
WITH pre_cot_data AS (
	...
	FROM 
		...
		`{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_precotizacion` tmp_pre
	GROUP BY 
		tmp_pre.id_adn
),
...
pre_pag_data AS (
	...
	FROM 
		...
		`{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_pago` tmp_pag
	WHERE
		(tmp_pag.estado > -1) 
...
FROM 
	...
	`{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_bitacora` b
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_agendamiento` a
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_estado` est
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_persona` per
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_seg_usuario` seg
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_usuario` usu
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_adn` adn
		ON (
			b.id_adn = adn.id_adn
		)
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_precotizacion` pre
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_cotizacion` cot
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_pago` pag
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_primer_pago` pripag
		ON (
	...
	LEFT JOIN `{project_id_raw}.ue2dbaprodrdjovv001__db_journeyvv.jvv_producto` pro
		ON (
	...
	LEFT JOIN `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_estado
		ON (
	...
	LEFT JOIN `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_tipdoca
		ON (
	...
	LEFT JOIN `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_tipdocc
		ON (
	...
	LEFT JOIN `{project_id_raw}.de__datalake.cfg_staging_maestra` cfg_origenld
		ON (
