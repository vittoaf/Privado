TRUNCATE TABLE `{project_id_stgmsrl}.stg_modelo_operaciones.tramite`; 
--WHERE periodo_creacion = DATE_TRUNC(CAST('{periodo}' AS date), MONTH);

INSERT INTO `{project_id_stgmsrl}.stg_modelo_operaciones.tramite`
WITH persona_data
AS (
	SELECT 
		a.id_persona,
...
tramites_hist_rentas as (
	...
	FROM `{project_id_raw}.de__canales.tramites_hist_rentas` thr 
...
tmp_tramite AS (
	...
	FROM `{project_id_raw}.bdwf__appnote.TRAMITE` trm
	...
	cot.id_cotizacion_origen AS numpoliza_buscar,
    ...
    cast(null as string) AS id_persona_via,
    cast(null as string) AS id_sede_via
	FROM `{project_id_stgmsrl}.stg_modelo_prospeccion.cotizacion` cot
    ...
    wk.numpoliza_buscar,
    wk.canalventa,
    wk.canalventa_homologado,
    wk.Id_Moneda,
    CAST(coalesce(wk.mnt_prima_emitida_bruta_anualizada,0.00) AS NUMERIC) AS mnt_prima_emitida_bruta_anualizada,
    CAST(wk.Fecha_Emision AS DATE) AS Fecha_Emision,
    wk.cod_cluster,
	wk.des_cluster,
    wk.est_estado_solicitud,
    wk.des_estado_solicitud,
    wk.id_persona_via,
    wk.id_sede_via
    FROM `{project_id_stgmsrl}.stg_modelo_operaciones.tramite_wk_tmp` wk
...
tmp_estructura_canal AS (
	SELECT DISTINCT 
		...
		dsc_canal
	FROM 
		`{project_id_stg}.stg_modelo_comercial.estructura_canal`
),
producto AS (
	SELECT 
		p.id_producto, 
	...
	FROM (
		SELECT 
			...
			po.id_origen
		FROM 
			`{project_id_stg}.stg_modelo_producto.producto` AS pr
...
mes_prod_data AS (
	...
	FROM `{project_id_raw}.de__canales.tramites_mes_produccion` a
	--WHERE a.CANAL='VIDA'
...
poliza_contratante AS (
	...
	FROM 
		`{project_id_stg}.stg_modelo_poliza.poliza` pol
	WHERE 
...
intermediario_mc AS (
	...
	FROM 
		`{project_id_stg}.stg_modelo_comercial.intermediario_mc` 
	WHERE 
...
stg_tramite AS (
	...
	FROM 
		tmp_tramite a 
		LEFT JOIN `{project_id_stg}.stg_modelo_persona.persona` b 
		...
		LEFT JOIN `{project_id_raw}.prod__acselx.TASA_CAMBIO` tc
